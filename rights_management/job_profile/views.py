...
def job_profile_delete(request, pk):
    ...
    try:
        ...
        app = 'No can delete this Job profile.'
        context = {'app': app}
        context['view_all'] = nav_bar_list(request.user)['view_all']
        return render(request, template_name='access_rights/no_can_add.html', context=context)
    ...
    app_current = 'Job Profile'
    context = {'name': job_profile.name, 'description': job_profile.description,
               'job_access_all': job_profile.job_access_all, 'in_department': job_profile.in_department,
